 ...
 */
function defineListeners({ getEtat, getCustomKeys, getVaisseau, getVaisseau2, getMode }) {
	...
	document.addEventListener('keydown', (event) => {
		...
		if (lastKeyPress[normalizedKey] && now - lastKeyPress[normalizedKey] < DOUBLE_TAP_DELAY) {
			...
			// Dash joueur 2 (mode duo, touches ZQSD)
		...
		// Tir : même touche que le solo, applique aux deux joueurs en duo
		if (normalizedKey === customKeys.shoot) {
			if (vaisseau1 && typeof vaisseau1.addBullet === 'function') {
				vaisseau1.addBullet(performance.now());
			}
			if (mode === 'duo' && vaisseau2 && typeof vaisseau2.addBullet === 'function') {
